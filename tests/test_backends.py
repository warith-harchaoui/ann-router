"""Behavioural tests for every backend adapter, uniform across engines.

For each backend whose dependency is installed, this module verifies the four
things the ANNIndex contract promises: recall@k against the exact ground truth
clears the versioned floor; a save/load round-trip preserves results; add/remove
work where the capability descriptor says they do (and raise
:class:`NotSupported` where it says they don't); and the capability descriptor is
self-consistent. Backends whose dependency (or server) is absent are *skipped*
with a reason, never failed — so the suite is green on a bare machine and
thorough on a full one.
...
def test_recall_above_threshold(name: str, dataset: dict) -> None:
    cls = _skip_if_unavailable(name)
    dim, k = dataset["dim"], dataset["k"]
    index = cls(dim=dim).build(dataset["corpus"])
    ids, dists = index.search(dataset["queries"], k)
...
@pytest.mark.parametrize("name", ALL_BACKENDS)
def test_save_load_round_trip(name: str, dataset: dict, tmp_path) -> None:
    cls = _skip_if_unavailable(name)
    if name in ("qdrant", "pgvector"):
        pytest.skip(f"{name} persists via its store (location/DSN), not save()/load()")
    dim, k = dataset["dim"], dataset["k"]
    index = cls(dim=dim).build(dataset["corpus"])
    before, _ = index.search(dataset["queries"], k)
...
@pytest.mark.parametrize("name", ALL_BACKENDS)
def test_add_matches_capability(name: str, dataset: dict) -> None:
    cls = _skip_if_unavailable(name)
    cap = cls.capabilities()
    dim = dataset["dim"]
    index = cls(dim=dim).build(dataset["corpus"][:1000])
    extra = dataset["corpus"][1000:1010]
    ids = np.arange(1000, 1010)
    if cap.supports_add:
        # Adding must grow the searchable set — the new ids become findable.
        index.add_with_ids(extra, ids)
        found, _ = index.search(extra[:1], k=5)
        assert extra.shape[0] == 10  # sanity on the batch
...
@pytest.mark.parametrize("name", ALL_BACKENDS)
def test_remove_matches_capability(name: str, dataset: dict) -> None:
    cls = _skip_if_unavailable(name)
    cap = cls.capabilities()
    dim = dataset["dim"]
    index = cls(dim=dim).build(dataset["corpus"][:2000])
    if cap.supports_remove:
        ...
        index.remove(np.array([target]))
        found, _ = index.search(query, k=10)
        assert target not in found[0]
    else:
        with pytest.raises(NotSupported):
            index.remove(np.array([0]))
